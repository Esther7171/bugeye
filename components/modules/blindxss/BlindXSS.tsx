...
import { BLIND_XSS_PAYLOADS, BLIND_XSS_LANDING_SPOTS } from '@/lib/blindxss';
import type { ModuleComponentProps } from '@/types';

export function BlindXSS({ onBack }: ModuleComponentProps) {
  const [collector, setCollector] = useState('https://your-id.xss.ht');

  const payloads = useMemo(
    () => BLIND_XSS_PAYLOADS.map((p) => ({ ...p, value: p.build(collector || '{{collector}}') })),
    [collector],
  );
  ...
  return (
    <div className="flex flex-col">
      <ModuleHeader
        title="BlindXSS"
        description="Generates blind-XSS payloads pointing at your collector. Generation only; nothing is injected."
        onBack={onBack}
      />
      <div className="flex flex-col gap-3 p-3">
        ...
        <div className="flex flex-col gap-1.5">
          <Label className="text-muted-foreground">Collector URL (XSS Hunter / interactsh / Burp Collaborator)</Label>
        ...
        <div className="flex flex-col gap-1.5 border-t border-border pt-3">
          <p className="text-xs font-medium">Where blind XSS commonly lands</p>
          <ul className="list-inside list-disc text-[11px] text-muted-foreground">
            {BLIND_XSS_LANDING_SPOTS.map((s) => (
              <li key={s}>{s}</li>
            ))}
          </ul>
        </div>
