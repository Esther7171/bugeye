...
const HEADER_SIGNATURES: HeaderSignature[] = [
  ...
  { vendor: 'Cloudflare', evidenceLabel: 'cf-mitigated header', test: (n) => n === 'cf-mitigated' },
  { vendor: 'Cloudflare', evidenceLabel: 'cf-apo-via header', test: (n) => n === 'cf-apo-via' },
  { vendor: 'Akamai', evidenceLabel: 'server: akamai', test: (n, v) => n === 'server' && /akamai|ghost/i.test(v) },
  ...
  { vendor: 'Akamai', evidenceLabel: 'x-akamai-transformed header', test: (n) => n === 'x-akamai-transformed' },
  { vendor: 'Akamai', evidenceLabel: 'akamai-x-cache header', test: (n) => n.startsWith('akamai-x-') },
  { vendor: 'AWS CloudFront', evidenceLabel: 'server: CloudFront', test: (n, v) => n === 'server' && /cloudfront/i.test(v) },
  { vendor: 'AWS CloudFront', evidenceLabel: 'x-amz-cf-pop header', test: (n) => n === 'x-amz-cf-pop' },
  { vendor: 'AWS CloudFront', evidenceLabel: 'via: CloudFront', test: (n, v) => n === 'via' && /cloudfront/i.test(v) },
  { vendor: 'Fastly', evidenceLabel: 'server: fastly', test: (n, v) => n === 'server' && /fastly/i.test(v) },
  { vendor: 'Fastly', evidenceLabel: 'x-fastly-request-id header', test: (n) => n === 'x-fastly-request-id' },
  { vendor: 'Fastly', evidenceLabel: 'via: varnish (Fastly)', test: (n, v) => n === 'via' && /varnish/i.test(v) },
  { vendor: 'Vercel', evidenceLabel: 'server: Vercel', test: (n, v) => n === 'server' && /vercel/i.test(v) },
  { vendor: 'Vercel', evidenceLabel: 'x-vercel-id header', test: (n) => n === 'x-vercel-id' },
  { vendor: 'Netlify', evidenceLabel: 'server: Netlify', test: (n, v) => n === 'server' && /netlify/i.test(v) },
  { vendor: 'Netlify', evidenceLabel: 'x-nf-request-id header', test: (n) => n === 'x-nf-request-id' },
  { vendor: 'Google Cloud', evidenceLabel: 'via: 1.1 google', test: (n, v) => n === 'via' && /\bgoogle\b/i.test(v) },
  { vendor: 'Azure Front Door / App Gateway', evidenceLabel: 'x-azure-ref header', test: (n) => n === 'x-azure-ref' },
  { vendor: 'Azure Front Door / App Gateway', evidenceLabel: 'server: Microsoft-Azure', test: (n, v) => n === 'server' && /microsoft-azure/i.test(v) },
  { vendor: 'Citrix NetScaler', evidenceLabel: 'via: NS-CACHE', test: (n, v) => n === 'via' && /ns-cache/i.test(v) },
...
const COOKIE_SIGNATURES: CookieSignature[] = [
  ...
  { vendor: 'Cloudflare', evidenceLabel: '__cflb cookie', namePattern: /^__cflb$/i },
  { vendor: 'Akamai', evidenceLabel: 'Akamai Bot Manager cookie (bm_*)', namePattern: /^bm_(sz|lso|mi|sv|s|v)$/i },
  { vendor: 'Akamai', evidenceLabel: 'ak_bmsc cookie', namePattern: /^ak_bmsc$/i },
  { vendor: 'Akamai', evidenceLabel: '_abck cookie', namePattern: /^_abck$/i },
  { vendor: 'Akamai', evidenceLabel: 'akaas_* cookie', namePattern: /^akaas_/i },
  ...
  { vendor: 'F5 BIG-IP', evidenceLabel: 'MRHSession / LastMRH_Session cookie', namePattern: /^(MRHSession|LastMRH_Session)$/i },
  { vendor: 'Citrix NetScaler', evidenceLabel: 'ns_af / citrix_ns_id cookie', namePattern: /^(ns_af|citrix_ns_id|NSC_)/i },
